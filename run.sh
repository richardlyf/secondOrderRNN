...
if [ "$1" = "parens_m4" ]; then
    CUDA_VISIBLE_DEVICES=0 python3 run.py --train-path=./data/mbounded-dyck-k/m4/train.formal.txt --valid-path=./data/mbounded-dyck-k/m4/dev.formal.txt --hidden-size 30 --embedding-dim 12 --log parens_m4 --log-every 10 --epochs 60 --lr 1e-4 --batch-size 1 --dropout 0
elif [ "$1" = "test_m4" ]; then
    ...
    CUDA_VISIBLE_DEVICES=0 python3 run.py --model mLSTM --train-path=./data/mbounded-dyck-k/m4/train.formal.txt --valid-path=./data/mbounded-dyck-k/m4/dev.formal.txt --hidden-size 30 --embedding-dim 12 --batch-size 1 --dropout 0 --lr 1e-4 --log parens_m4_mLSTM --log-every 10
else
